...
for iter = 1:10
    ...
    % ahhh i see what they do now with x0=rand(1,3), they multiply beta
    % parameter with 50 inside the computellh function, not sure if this is
    % for programming convenience in some way or if it has an actual effect
    % somewhere. confused me for a bit
    ...
    % of course is likely
    % ah okay, i was missing something; matlab only prints 4 decimals so to
    % me they all looked the same. so we can do num2str(pars(:,end), 20) to
    % see 20 decimals and there see that for example the one at index 4
    % differs around 8th decimal
    [mf,i]=min(pars(:,end));
